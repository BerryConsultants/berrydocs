...
ui <- fluidPage(
  ...
  br(),
  titlePanel(h2("Plot a Subjects Prior Predictive", align = "center")),
  fluidRow(
...
getLowerMedianUpper = function(earlyVisitVal, alpha = c(0,1), beta = c(0,1), lambda = c(1,1)) {
  ...
  dev2 = sample(deviates)
  
  samps = (deviates*distMeanFinal[2] + distMeanFinal[1]) + dev2*lambda[1]
...
server <- function(input, output, session) {
  ...
  output$dataInputTable = renderDT(datatable(df, 
    options = list(
      dom = "t",
      autoWidth = TRUE,
      columnDefs = list(list(width = '150px', targets = 1:8))
    ),
    rownames = TRUE,
   # escape = FALSE,
    selection = 'none',
    editable = list(target = "cell", disable = list(columns = 1))
    ) |> formatRound(2, digits = 4) |> formatRound(3:8, digits = 2)
  )
  ...
  observe({
    ...
    if(nv > nrow(df)) {
      for(i in 1:(nv-nrow(df))) {
        df <<- rbind(df, setNames(data.frame(matrix(c(1+nrow(df), df[nrow(df),-1]), nrow = 1)), names(df)))
      }
    } else if(nv < nrow(df)) {
      df <<- df[1:nv,]
    }
  ...
  output$priorPredictive = renderPlot({
    ...
    tempDF = df
    dataToPlot = getLowerMedianUpper(tempDF[lvIndex,2],
                                     alpha = c(tempDF[lvIndex,3], ifelse(input$fixAlpha, 0, tempDF[lvIndex,4])),
                                     beta = c(tempDF[lvIndex,5], ifelse(input$fixBeta, 0, tempDF[lvIndex,6])),
                                     lambda = c(tempDF[lvIndex,7], tempDF[lvIndex,8]))
    
    tempDF$visitKnown = ifelse(1:nrow(tempDF) <= lvIndex, "included", "excluded")
    p1 = ggplot() + 
      geom_point(dat = tempDF, aes(x = Visits, y = VisitResponse, color = visitKnown)) + 
