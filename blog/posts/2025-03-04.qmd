...
I gave a talk at [ADMTP 2025](https://admtp.github.io/ADMTP2025/){target="_blank"} on using Machine Learning to optimize a trial design. Click [here](attachments/2025-03-04-SlidesTP.pdf){target="_blank"} to download the slides. As trials get more complex, so they acquire more design parameters that need setting, without a priori clear values that should be used and there is a considerable parameter space that needs to be explored. Can this be done for us automatically using the latest Machine Learning (ML) software, evaluating our possible design choices using simulation?
