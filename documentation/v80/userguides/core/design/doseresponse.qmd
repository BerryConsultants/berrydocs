...
# Continuous, Dichotomous, Time-To-Event, and Ordinal

With the exception of two dose response models specific to a dichotomous
endpoint, and the Dirichlet modeling capability for an ordinal endpoint,
the same dose response modeling facilities are available for all endpoints.
...
In the ordinal case the final endpoint for the $i^{th}$ participant can take on any of $K$ different values $1, 2, \ldots, K$. If the $i^{th}$ participant has been allocated to the control arm $(d_i = 0)$ then
$$ Y_i \sim \mbox{Categorical}(p^0_1,p^0_2,\ldots,p^0_K), $$
that is:
$$ \mbox{Pr}\left\{Y_i = k | d_i = 0 \right\} = p^0_k$$
where $0 \leq p^0_k \leq 1$, $\sum_{k=1}^K p^0_k = 1$.

If the $i^{th}$ participant is assigned to arm $d_i = d$, then
$$ \mbox{Pr}\left\{Y_i = k | d_i = d \right\} = p^d_k$$
When the user selects ordinal modeling with independent Dirichlet distributions, then the ordinal outcome
probabilities $p^d_k$ for the other arms can vary freely (they are not functions of the control arm probabilities).

When the user has selected the cumulative logistic family, the $d^{th}$ arm has a log-odds ratio parameter $\theta_d$
such that
$$\mbox{logit Pr}\{Y_i \leq k | d_i = d\} = \mbox{logit Pr}\{Y_i \leq k | d_i = 0\} - \theta_d = \alpha_k - \theta_d
\mbox{ for } k=1,\ldots,K-1$$
where $\mbox{logit}(p) = \log\{p/(1-p)\}$ and where we have defined
$\alpha_1 \leq \alpha_2 \cdots \leq \alpha_{K-1}$ by
$\alpha_k = \mbox{logit} \left( \sum_{j=1}^k p^0_j \right)$.

If $\theta_d > 0$, small values of the ordinal endpoint are less likely for arm $d$ than they are for control.
In other words, if small values of the ordinal endpoint are desirable, then an arm $d$ with $\theta_d < 0$ is
beneficial compared to control.  On the other hand if large values of the ordinal endpoint are desirable, then
$\theta_d > 0$ implies that arm $d$ is beneficial relative to control.
...
If the response is ordinal and the user has selected cumulative logistic modeling, $\theta_d$ is
the log odds ratio for arm $d$ compared to the control arm.  The control arm parameter $\theta_0 = 0$ by definition.

If the user has selected independent Dirichlet modeling, then there is no parameter $\theta_d$.

As of the initial release of the FACTS Ordinal Engine, the control arm is never included in the dose-response model,
and two-dimensional models are not supported.

The prior distribution for the control arm probabilities is
$$ (p^0_1, p^0_2, \ldots p^0_K) \sim \mbox{Dirichlet}\left( a^0_1, a^0_2, \ldots, a^0_K \right) $$
where $a^0_k>0$ for $k=1,2,\ldots,K$.

If the user selects independent Dirichlet modeling, then each arm $d$ has its own Dirichlet prior distribution
$$ (p^d_1, p^d_2, \ldots p^d_K) \sim \mbox{Dirichlet}\left( a^d_1, a^d_2, \ldots, a^d_K \right) $$
If non-informative priors for the $a^d_k$s are desired, Berger, Bernardo, and Sun
("Overall Objective Priors", Bayesian Analysis 2015), recommend using $a^d_k$ of approximately $0.8/K$ for $k=1,2,\ldots,K$.
A common informative approach is to interpret the $a^d_k$ as numbers of patients observed in each category in a similar previous study.
:::
...
$$\theta_d \sim \text{N}(\mu_d, \tau_d^2)$$

Where $\mu_d$ and $\tau_d$ are specified
...
initial dose $d_I=1$ if there is no control or control is
included in the dose response model, and $d_I=2$ if the
control arm is modeled separately.

The dose response of the first dose, $d_I$, has a prior of:

$$\theta_{d_I} \sim N\left(\mu_{I},\tau^2_{I}\right)$$

where $\mu_I$ and $\tau_I^2$ are
specified directly in FACTS. Subsequent dose response estimates
$\theta_{d_{I+1}}, \ldots, \theta_D$ have priors
...
$$\theta_d \sim N\left(\theta_{d-1},\tau^2_{d-1}\right) \text{ for } d=d_I+1, \ldots, D$$
...
$$\tau^{2}\sim IG\left( \frac{n_\tau}{2},\frac{m_\tau^2 n_\tau}}{2} \right)$$

where $m_\tau$ and $n_\tau$ are specified in the Dose
...
$$\sigma^{2}\sim IG\left( \frac{n_\sigma}{2},\frac{m_\sigma^{2}n_\sigma}{2} \right)$$

where $m_\sigma$ and $n_\sigma$ are specified on the Dose
...
When using the NDLM model or any of its alternatives
...
## Monotonic Normal Dynamic Linear Model
...
Let doses $d = d_I, \ldots, D$ be doses in the dose response model and
$\theta_d$ be the estimated dose response for dose $d$. The
initial dose $d_I=1$ if there is no control or control is
included in the dose response model, and $d_I=2$ if the
control arm is modeled separately. The following model is the
monotonically increasing NDLM:

$$\theta_{d'} \sim N\left(\mu_{I},\tau^2_{I}\right)$$

and

$$\theta_d \sim N^+\left(\theta_{d-1},\tau^2_{d-1}\right) \mbox{ for } d = d_{I+1}, \ldots, D,$$
where
$$\tau^2_{d-1}=\tau^2\left(\nu_d-\nu_{d-1}\right)$$
as in the standard NDLM, and
...
## Second Order Normal Dynamic Linear Model

The second order NDLM serves as a “smoother” similar to the
standard NDLM, however it tends to smooth toward a linear fit with a slope. The
standard NDLM tends to shrink each dose’s fit toward its neighbors, while
the second order NDLM prefers any trend in the neighbors.

Let doses $d=d_I, \ldots, D$ be doses in the dose response model and $\theta_d$ be the estimated dose response for dose $d$. The
initial dose $d_I=1$ if there is no control arm or control
is included in the dose response model, and $d_I=2$ if the
control arm is modelled separately. The initial dose $d_I$ is modeled:

$$\theta_{d_I} \sim N\left(\mu_{I},\tau^2_{I}\right)$$

where $\mu_I$ and $\tau_I$ are specified
directly in FACTS.

In the case of a time-to-event endpoint, the initial dose $d_I$ is the control arm, and has a $\theta_{d_I}= 0$ by definition, so no prior distribution is needed.
...
$$\theta_{d_I+1} - \theta_{d_I} \sim N\left(\mu_{II},\tau^2_{II}\right)$$

Successive doses are then modeled based on differences in slope between
the dose and the two doses below them. Let:

$$\theta_{d} = \theta_{d - 1} + \Delta \nu_{d}\omega_d + \frac{\Delta_{d}}{\Delta_{d - 1}}\left( \theta_{d - 1} - \theta_{d - 2} \right)$$

for doses $d=d_I+2,\ldots,D$, where
...
there is evidence that it is appropriate. See the [Hierarchical Logistic](#hierarchical-logistic)
and [Sigmoid](#sigmoid-model) (E<sub>max</sub>) models for dose response models with a
similar pattern, but slightly more flexibility in shape.

## Hierarchical Logistic

The <span class="custom-tooltip"> hierarchical logistic model <span class="tooltip-text">Scott Berry's favorite dose response model.</span> </span> is an extension of the 3-parameter
