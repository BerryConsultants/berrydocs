...
`C:\Program Files (x86)\BerryConsultants\FACTS 8.0.0\ReportGenerator`). The files within this folder with file extensions of ".R", and ".Rmd" are the ones required by the Report Generator. If an updated version of the Report Generator is made available either as a bug-fix or intermediate release, it will consist of updated ".R" and ".Rmd" files. Simply replacing the corresponding files in the original installation folder will deploy the updated version.
