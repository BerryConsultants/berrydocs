---
title: "CRM"
subtitle: "Model-Based Dose Escalation (N-CRM / BLRM)"
...
| True Ppn. Tox | 1 | This is the average true probability of toxicity over the simulations. The ‘true probability of toxicity’ in a simulated trial is the average of the probabilities of toxicity all the subjects are exposed to, given the toxicity response profile that we are simulating from. |
...
| Mean Lost Subjects | 1 | If the trial uses open enrollment, this is the number of subjects that could not be allocated a dose because the number of subjects treated but not yet complete had reached the specified “Maximum subjects without final result” limit. Otherwise the value is 0. |
| Ppn(All Tox) | 1 | The proportion of the simulations that stopped because all doses were too toxic. |
| Ppn Selected MTD Under | 1 | The proportion of the simulations where true toxicity rate of the selected MTD dose lay in the “Underdosing” toxicity range. |
| Ppn Selected MTD Target | 1 | The proportion of the simulations where true toxicity rate of the selected MTD dose lay in the “Target” toxicity range. |
| Ppn Selected MTD Excess | 1 | The proportion of the simulations where true toxicity rate of the selected MTD dose lay in the “Excess” toxicity range. |
| Ppn Selected MTD Unacc | 1 | The proportion of the simulations where true toxicity rate of the selected MTD dose lay in the “Unacceptable” toxicity range. |
...
| True Ppn. Eff | 1 | Included if efficacy is being simulated. This is the average true probability of efficacy over the simulations. |
| Ppn All Toxic Early Stop | 1 | The proportion of the simulations that stopped early (i.e. before reaching the maximum sample size) because all doses were too toxic. |
| Ppn Reached Max Sample Size | 1 | The proportion of the simulations that stopped because the maximum sample size had been reached. |
...
| Model OSD | 1 \[simulations only\] | The index of the dose selected as the Optimal Safe Dose (OSD). If there is no efficacy endpoint, this will always be the MTD. If there is no safety endpoint, this will always be the MED. If there are both safety and efficacy endpoints, the OSD is either the MED, if MED <= MTD; otherwise there is OSD (a dose that is safe but not efficacious does not qualify as an "optimal" dose). |
