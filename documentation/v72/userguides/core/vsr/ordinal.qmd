...
Dose response profiles can be added and deleted.  For each profile
the user may use either of these specifications: 

1.  The outcome probabilities for each treatment arm (if "Use proportional odds" is unchecked).  
FACTS allows the user to supply unnormalized probabilities (for example, counts) and will convert 
the input into a vector of probabilities that sum to one.  

2.  The outcome probabilities for the control arm (which can be entered unnormalized), and an odds ratio 
for each other arm.  If the normalized probabilities for the control arm are denoted by 
$(p^0_1,p^0_2,\ldots,p^0_K)$ and the odds ratio for the $d$th arm is $\exp(\theta_d)$, 
then the probabilities for the $d$th arm are given by 
