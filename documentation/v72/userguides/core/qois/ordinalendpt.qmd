...
The Ordinal Engine has the following different frequentist final
...
Podcast by Scott Berry. FACTS simulations can be useful in demonstrating
...
-   Predictive probabilities for the Wilcoxon test: Both future trial
    ...
    has a better outcome than a control arm patients, and then correctly
...
probabilities, see the core QOIs page. In the present version of the
FACTS Ordinal Engine, current trial predictive probabilities are only
available for the full sample size intended at the end of the trial.
