...
The QOI (Quantities Of Interest) tab allows the user to specify the Bayesian
posterior quantities and frequentist p-values that are eligible to be used
to make decisions in the trial as well as being output to the simulations
results files.
...
1.  A probability calculated independently **for each dose**. There are
    ...
    a.  The posterior probability that the estimate of the response of
    ...
    b.  The predictive probability/conditional power of success (achieving frequentist statistical significance) in the current trial or a future trial comparing the estimate of response on the dose against that on the Control arm.

    c.  P-values for each dose comparing the estimate of response on
...
A Posterior Probability is specified by providing a value for each of the
following:
...
-   The comparison can include a delta, which is the absolute value to
    be compared against if t    the difference relative to the comparison arm is compared to.
he comparison is absolute, or a value that
...
There are a couple of aspects of posterior probabilities that are slightly
different for posterior probability QOIs.

First, in FACTS Core TTE a Posterior Probability QOI (a Bayesian comparison
of estimates of response) can be used to compare Hazard Ratios or
Hazard Rates. In their descriptions Hazard Ratio QOIs use "HR" (e.g.
"HR~d~ \< 1") and Hazard Rate QOIs use $\lambda$ (e.g. "$\lambda_d$ \<
$\lambda_{control}$"). If there are hazard rate QOIs defined, then FACTS
restricts the hazard model (defined on the Design \> Hazard Model
tab) to use just a single segment (so that there is just one lambda
to compare). This is set in the "Com are" box of the QOI definition
window:

![Specifying a posterior probability QOI based on hazard rate.](../coreUGattachments/tte/media/image38.png){#fig-hazratesQOI}

Additionally, in FACTS Core TTE when defining a Posterior Probability QOI (a
Bayesian comparison of estimates of response) or a Target QOI (a
Bayesian assessment of which treatment arm is most likely to fulfill
a 'target' criteria) -- the user can select whether the evaluation
uses the Final Event endpoint, or the Predictor.

![Specifying a posterior probability QOI based on the predictor endpoint.](../coreUGattachments/tte/media/image39.png){#fig-predictorposteriorprob}
...
different, and the implications need to be <span class="custom-tooltip"> carefully understood <span class="tooltip-text">Simulating its use in FACTS is a good way to achieve that
understanding!</span> </span>. Where the term CSD is used in this document, it should be taken to refer
...
p-value test type – unadjusted, Bonferroni or Dunnett's [@dunnett1955multiple] and the (one
...
See the PASS documentation [@PASS] or the SAS documentation [@SAS] for a complete
...
used (unadjusted, Bonferroni, Dunnett's [@dunnett1955multiple], or Trend Test), and how missing
data is to be handled (ignored, LOCF, <span class="custom-tooltip"> BOCF <span class="tooltip-text">Only if continuous endpoint and baseline is being simulated.</span> </span>, and <span class="custom-tooltip"> missing is failure <span class="tooltip-text">Dichotomous endpoint only.</span> </span>). If a control arm is present, p-values are comparisons
...
normal distribution, and at least <span class="custom-tooltip"> 5 success and 5 failures <span class="tooltip-text">Sometimes this is said to be 10 and 10, or 15 and 15. Your rule of thumb can be based on your comfort level for allowing CLT to kick in.</span> </span> should be
...
Rather than model estimated quantities, operational QOIs are based on the current state of the simulated trial, rather than accumulating efficacy data. Operational QOIs are meant to be combined with other QOIs to make decisions that vary based on the enrollment or information collected.

Operational quantities include:

 - Number Enrolled
 - Number Complete (any visit)
 - Number with opportunity to complete (any visit)
 - Number of Events (TTE only)
 - Exposure (TTE only)
 - Effective Dose Strength
 
Like the previously described model estimated QOIs, each arm will have its own quantity for an operational QOI. To use operational QOIs in decision making, a decision QOI must be created from the operational QOI - exactly as with other QOIs.

In addition to being used in decision making, creating an operational QOI guarantees that the calculated quantity is reported in the simulation output. This can be useful to force reporting of the number of subjects that have completed a specific visit, which isn't reported by default.

::: {.panel-tabset}

### Continuous/Dichotomous

![Add an operational QOI.](../coreUGattachments/CoreUserGuide/media/OpQOIS_CD.png){#fig-decqoipostprob}

### Time-to-Event

![Add an operational QOI.](../coreUGattachments/CoreUserGuide/media/OpQOIS_TTE.png){#fig-opQOItte}
