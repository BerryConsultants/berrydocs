...
options are as per the [continuous and dichotomous endpoint standard dos response options](../design/doseresponse.qmd) (except use of BAC for the predictor is not supported).
...
For the relationship to endpoint model, $Z_1 \sim \text{PWExp}(\lambda_{1s}*\theta_{1d})$ and $Z_2 \sim \text{Exp}(\lambda_{2d})$, with priors
