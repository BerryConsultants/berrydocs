...
predictor $Z$ by assuming $T\mid Z \sim \Exp(\lambda_d e^{\beta Z})$, where
...
predictor, with $$T\mid Z \sim \Exp(\lambda_d e^{\betaZ})$$. In this
situation the working model is simpler to understand, as
$$T\mid (Z=0) \sim \Exp(\lambda_d)$$ and $$T \mid (Z=1) \sim Exp(\lambda_{d\beta})$$.
