...
![The study tab for a dichotomous trial.](../coreUGattachments/dichotomous/media/image3.png){#fig-studyd}
...
If this option is selected, then the endpoint is actually simulated as a dichotomized continuous response
-- e.g. a "responder analysis" where a subject with a change
from baseline above (or below) a certain threshold is a 'responder' and
otherwise is not.

When using a dichotomized response, the longitudinal analysis
is on the underlying continuous response rather than the dichotomous responder endpoint. After imputing subject values based on intermediate subject data they are dichotomized.

#### Use restricted Markov model

When using the restricted Markov
model a subject can be in one of 3 states "Success",
"Failure" or "Stable." Once the subject has become a
"Success" or "Failure" they are [fixed in that state](## "Success and Failure are called "absorbing states."), and it will be their final outcome.

All subjects are initially in the "Stable" state and may or
may not transition to another state during follow-up. If a subject
finishes follow-up in the "Stable" state, they must be classified as failure or success.
...
Indicate whether a response indicates the subject improving
or worsening (and thus whether a higher or lower response rate
is a good thing). The value provided here informs the [direction
of frequentist hypothesis tests](## "All FACTS frequentist tests
are 1-sided.")
and the evaluation of QOIs like [Pr(Max)]{.fake-code-block} and predictive probabilities.
...
![The control for setting up the visit schedule.](../coreUGattachments/dichotomous/media/image15.png){#fig-scheduled}
...
![The Study > Treatment Arms tab when using a 2D treatment arm model.](../coreUGattachments/dichotomous/media/image17.png){#fig-2dtreatmentd}
...
![The dosing grid for a 2D dosing model with 2 row factor levels and 4 column factor levels.](../coreUGattachments/dichotomous/media/image18.png){#fig-2ddosingd}
