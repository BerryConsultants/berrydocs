...
-   In the dichotomous case the final endpoint of the
    ...
    *P*<sub>*d*</sub> is modelled on the logit scale, so
    $P_{d} = \frac{e^{\theta_{d}}}{1 + e^{\theta_{d}}}$, and thus
...
$\delta$<sub>*d*</sub> = *ν*<sub>*d*</sub> − *ν*<sub>*d* − 1</sub> and
$\delta$<sub>*d* − 1</sub> = *ν*<sub>*d* − 1</sub> − *ν*<sub>*d* − 2</sub>.
...
*θ*<sub>*d*</sub> = *θ*<sub>0</sub> + *S* • $\delta$
...
1.  $\delta$ represents the maximal change in response from the zero-strength
    dose. It is restricted to be positive:

> $\delta$ ∼ *N*<sup>+</sup>(*μ*<sub>$\delta$</sub>, *σ*<sub>$\delta$</sub><sup>2</sup>)
...
1.  $\alpha$ determines the rate of change of the dose response curve for
...
> $\alpha$ ∼ *L**N*<sup>\*</sup>(*μ*<sub>*α*</sub>, *σ*<sub>*α*</sub><sup>2</sup>)
...
to constrain the values of $\alpha$ and *β* by utilizing small standard
...
The exponent parameter $\alpha$<sub>3</sub> allows the change in slope at the
...
for all doses *d* in the model. Both $\alpha$ and *β* are given normal prior
distributions:
...
where the $\alpha$ and *β* parameters are as in the linear model, with prior
distributions
...
Thus $\alpha$<sub>0</sub> is the response at the control combination,
*α*<sub>1</sub> is the linear coefficient of the response to the column
factor strengths *ζ*<sub>*c*</sub>, and $\alpha$<sub>2</sub> is the linear
coefficient of the response to the row factor strengths
*η*<sub>*r*</sub>.

The user has the option to simplify the model and exclude the
interaction term $\alpha$<sub>3</sub>, which is the coefficient of the
...
The parameter $\alpha$<sub>*t*</sub> is the intercept of the model for visit
...
*y*<sub>*i**t*</sub> ∼ *e*<sup>*α*<sub>*t*</sub></sup>(*θ*<sub>*d*</sub> + $\delta$<sub>*i*</sub>) + *N*(0, *λ*<sub>*t*</sub><sup>2</sup>)

The imputed final response (visit *T*) for the *i*<sup>*t**h*</sup>
subject, having been randomized to the *d*<sup>*t**h*</sup> dose is
modeled as:

*Y*<sub>*i**T*</sub> ∼ *θ*<sub>*d*</sub> + $\delta$<sub>*i*</sub> + *N*(0, *λ*<sub>*T*</sub><sup>2</sup>)

(i.e. $\alpha$<sub>*T*</sub> is 0).
...
> $\delta$<sub>*i*</sub> is the estimated patient level random effect around
> the mean final response (*θ*<sub>*d*</sub>) for the dose *d* that
> patient *i* is randomized to.
>
> $\alpha$<sub>*t*</sub> is a scaling parameter that determines the
> proportion of the final response that is observable at visit *t*. A
> value of $\alpha$<sub>*t*</sub> = 0 indicates that the expected value of
> early visit *t* is equal to the estimated final visit mean
> *θ*<sub>*d*</sub>. A value of $\alpha$<sub>*t*</sub> = −0.69315 indicates
...
The prior for $\alpha$<sub>*t*</sub> is a normal distribution with a user
specified the mean and standard deviation:

*α*<sub>*t*</sub> ∼ *N*(*α*<sub>*μ*</sub>, *α*<sub>*σ*</sub><sup>2</sup>),

The prior for the $\delta$<sub>*i*</sub> terms is a normal distribution with
a mean of 0 and variance *τ*<sup>2</sup>.

$\delta$<sub>*i*</sub> ∼ *N*(0, *τ*<sup>2</sup>)
...
> $\alpha$<sub>*t*</sub> mean of -2, SD of 2, … so the prior ~70% interval
> for $\alpha$<sub>*t*</sub> is between -4 and 0 (+/1 1 SD) and thus the
...
distributions of $\alpha$<sub>*t*\*</sub> and *β*<sub>*t*\*</sub> as:
...
1 for $\alpha$<sub>*t**x*</sub> (or 0 for *β*<sub>*t**x*</sub>) and the
response at time *t* is *x*.

The $\alpha$<sub>*t**x*</sub> and *β*<sub>*t**x*</sub> parameters are
independently estimated using only patients in their model instance, and
may or not have identical priors $\alpha$<sub>*μ*\*</sub> and
