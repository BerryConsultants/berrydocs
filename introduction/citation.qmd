...
If the webpage you are referencing does contain an explicit author, like a blog post would, use that author as **author**, like so: *For more information on Optimizing Group Sequential Designs using Machine Learning, see the FACTS Knowledge Hub [@FACTSKH2]*. Here are the most common citation formats using the above example (please substitute the title and link with the actual page you want to cite):
